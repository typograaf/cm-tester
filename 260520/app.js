...
// This build has no toggle UI at all — Diagonal ABPR (ss08) is the
// single fixed active set, everything else off. ss08 is driven
// through state.featureState directly (see init), so nothing needs
// to be force-appended here.
const ALWAYS_ON_FEATURES = [];
// The one set that's always on in this build.
const FIXED_ACTIVE_SET = "ss08";
...
function renderSSPills() {
  if (!state.font) return;
  if (!ssPillsEl) return;  // no toggle UI in this build
...
function refitStage(opts = {}) {
  ...
  // Type is sized the same regardless of background — the stacked
  // layout leaves enough room to read the photo subject without
  // shrinking the headline in image mode.
  const ceiling = max;
...
async function init() {
  ...
  // is filtered out of state.font.features by HIDDEN_FROM_UI, so we
  // add its featureState key explicitly — applyTypography emits it
  // from there and the detail panel focuses it.
  for (const feat of state.font.features) {
    state.featureState[feat.tag] = false;
  }
  state.featureState[FIXED_ACTIVE_SET] = true;
  state.focusedSS = FIXED_ACTIVE_SET;
